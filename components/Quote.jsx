...
module.exports = class RichQuote extends React.Component {
  ...
  async search() {
    function searchAPI (content, author_id, max_id, id, dm, asc) {
      return new Promise((resolve, reject) => {
        ...
        const opts = { author_id,  max_id, content, include_nsfw: true };
    ...
    this.setState({ ...this.state, searchStatus: 'loading' });
    ...
    if (result.messages.length !== 0) {
      ...
      if (!message) {
        this.setState({ ...this.state, searchStatus: 'error' });
        return;
      }

      this.setState({ ...this.state, searchStatus: 'done' });

      const messageRoute = `/channels/${this.props.channel.guild_id || '@me'}/${this.props.channel.id}/${message[0].id}`

      transitionTo(messageRoute);
    } else this.setState({ ...this.state, searchStatus: 'error' });
  }
  ...
  openUserContextMenu (event) {
    ...
    if (!guildId) return contextMenu.openContextMenu(event, (props) => React.createElement(GroupDMUserContextMenu, {
      ...props, user: userStore.getUser(userId),
      channel: this.props.channel
    }));

    contextMenu.openContextMenu(event, (props) => React.createElement(GuildChannelUserContextMenu, {
      ...props, user: userStore.getUser(userId),
      guildId, channelId: this.props.channel.id,
      showMediaItems: false, popoutPosition: 'top'
    }));
  }
